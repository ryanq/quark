        ...
        impl BitIndex for $type
        {
            fn bit(&self, index: usize) -> bool {
                self.checked_shr(index as _)
                    .unwrap_or_else(|| if *self < 0 { 1 } else { 0 }) & 1 == 1
            }
            ...
            fn bits<Idx: RangeBounds<usize>>(&self, index: Idx) -> Self {
                ...
                match (shift, mask) {
                    (Some(s), Some(m)) => self.checked_shr(s as _).unwrap_or_else(|| {
                        if *self < 0 { (0 as Self).wrapping_sub(1) } else { 0 }
                    }).mask_to(m),
...
mod test {
    ...
    impl RangeBounds<usize> for RangeEI {
        fn start_bound(&self) -> std::ops::Bound<&usize> { std::ops::Bound::Excluded(&self.0) }
        fn end_bound(&self) -> std::ops::Bound<&usize> { std::ops::Bound::Included(&self.1) }
    }
